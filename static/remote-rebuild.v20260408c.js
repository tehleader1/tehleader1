(() => {
  ...
  let recordChunks = [];
  const boardAudio = { voice: null, beat: null, adlib: null };
  ...
  function renderSettings() {
    ...
    $("srOpenFullSettings").onclick = () => {
      $("srSettingsStatus").textContent = "Full Settings: password, push, address, credentials, current payments, and premium verification are now the active lane.";
      activateRoute("floatProfileBox");
    };
  }
  ...
  function renderDiary() {
    ...
    const historyLines = ["Chat history empty right before they start.", "Aria is ready for hair talk.", "Jake is waiting for studio help."];
    box.innerHTML = `
      <div class="support-rebuild-shell">
        <div class="support-rebuild-card">
          ...
          <div class="support-rebuild-grid two" style="margin-top:12px">
            ...
            <div class="support-rebuild-card" style="padding:12px">
              ...
              <div class="support-rebuild-note">Fixed above the history and ready for hair problem recording.</div>
          ...
          ${state.liveMode ? `<div class="support-rebuild-card" style="margin-top:12px"><div class="support-rebuild-title">Live Session</div><div class="support-rebuild-note">Public display on. Hearts, likes, comments, and Fast Pay sit here while live is active.</div></div>` : ""}
        </div>
      </div>`;
    $("srSendSocialBtn").onclick = openPlatforms;
    $("srDiaryLevel").onchange = (e) => { state.diaryLevel = e.target.value; saveState(); };
    $("srHandsFreeBtn").onclick = () => alert("Hands-Free Mode armed for diary recording.");
        ...
        <div class="support-rebuild-card">
          ...
          <div class="support-rebuild-grid two">
            ...
            <div class="support-rebuild-card" style="padding:12px">
              ...
              <div style="margin-top:12px">${["Clean image under pressure.","Hair-aware and socially ready.","Profile saves real presentation."].map((s)=>`<div class="support-rebuild-line">${s}</div>`).join("")}</div>
            </div>
          </div>
          <div class="support-rebuild-grid two" style="margin-top:12px">
            <div class="support-rebuild-card" style="padding:12px">
  ...
  function renderMap() {
      ...
      <div class="support-rebuild-shell">
        <div class="support-rebuild-card">
          ...
          <div class="support-rebuild-perks" style="margin-top:12px">
            ${view.perks.map((perk)=>`<button class="support-rebuild-btn ghost" data-perk="${perk}">${perk}</button>`).join("")}
          </div>
    ...
    box.querySelectorAll("[data-perk]").forEach((btn) => btn.onclick = () => {
      const perk = btn.dataset.perk.toLowerCase();
      const next = perk.includes("studio") || perk.includes("voice") ? "floatBoardsBox" : perk.includes("profile") ? "floatAssistantBox" : perk.includes("live") || perk.includes("diary") ? "floatSettingsBox" : "floatLiveBox";
      $("srMapStatus").textContent = `${btn.dataset.perk} is routing you toward ${ROUTES[next]}.`;
      activateRoute(next);
    });
  }
      ...
      <div class="support-rebuild-shell">
        <div class="support-rebuild-card">
          ...
          <div class="support-rebuild-grid three" style="display:grid;gap:12px;grid-template-columns:1fr">
            ${["voice","beat","adlib"].map((board)=>`<div class="support-rebuild-board"><div class="support-rebuild-row"><strong>${board.toUpperCase()} BOARD</strong><button class="support-rebuild-btn ghost" data-board="${board}">Select</button><input type="file" accept="audio/*,video/*" data-upload="${board}"></div><div class="support-rebuild-note" id="srBoardName_${board}">${board}-track.wav</div><div class="support-rebuild-wave"></div></div>`).join("")}
          </div>
          <div class="support-rebuild-grid two" style="margin-top:12px">
            <div>
              <label class="support-rebuild-note">FX Settings</label>
              <select class="support-rebuild-select" id="srStudioFx"><option>Echo</option><option>Reverb</option><option>Fade In</option><option>Fade Out</option><option>Bass</option><option>Treble</option><option>Deep Voice</option><option>Opera Voice</option><option>Slow Motion</option></select>
            </div>
